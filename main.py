...
KALSHI_PRIVATE_KEY = b"""YOUR_KALSHI_PRIVATE_KEY"""
...
BET_DOLLARS      = 20    # dollars to risk per trade
BALANCE_FLOOR    = 140.0   # halt if balance at or below this

CLOSE_WATCH_SECS = 300    # start watching price this many secs before close
...
def main():
    ...
    try:
        while True:
            ...
            # Sleep until watch window
            wait = secs_left - CLOSE_WATCH_SECS
            if wait > 0:
                log(f"Sleeping {wait:.0f}s until watch window")
                time.sleep(wait)
